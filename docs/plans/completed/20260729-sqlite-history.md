...
- [x] создать `Processes.sq`: справочник `process` с `UNIQUE(pid, started_at)`, `process_sample` с FK на `sample`, индекс на `sample_id` — ⚠️ FK на `application` в итоговой схеме нет, см. заметку ниже и задачу 4
...
>
> ⚠️ **Решение отменено во внешнем ревью после задачи 14.** Клауза, дописанная в задаче 4, снята
> обратно: из двух колонок `process_sample` `REFERENCES` в итоговой схеме несёт только `sample_id`
> (`application_sample.application_id` свой внешний ключ сохранил, см. заметку к задаче 4).
> Обоснование — в
> заметке к задаче 4 и в комментарии `Processes.sq:26-34`.
...
- [x] ➕ дописать `REFERENCES application(id)` к `process_sample.application_id` в `Processes.sq` — в задаче 3 клауза не компилировалась, таблицы ещё не было; ⚠️ во внешнем ревью после задачи 14 клауза снята обратно, см. заметку ниже
...
>
> ⚠️ **`REFERENCES application(id)` у `process_sample.application_id` отменён во внешнем ревью после
> задачи 14** — вместе с так и не поставленным FK на `process`. Причина не в генерации, а в цене на
> проде: при `foreign_keys = ON` SQLite доказывает отсутствие детей у удаляемого родителя сканом
> дочерней таблицы, по одному скану на каждую удаляемую строку справочника, и дочерняя здесь —
> `process_sample`, самая большая таблица схемы. Замер на её форме (800 000 дочерних строк, 200
> сирот): 5.66 s CPU с клаузами против 0.13 s без них, и всё это внутри пишущей транзакции
> ретеншна, пока следующий сэмпл ждёт. Взамен не покупается ничего: сироты собираются явным `NOT IN`
> ровно по этим колонкам, так что строка, которую `REFERENCES` отказался бы осиротить, — это строка,
> которую `NOT IN` и не выбирает. `application_sample.application_id` FK **сохранил**: эта таблица на
> порядок меньше, и в ней id приложения обязателен. Итоговая схема — `Processes.sq:26-38`.
...
>
> Шов для драйвера — это публичный конструктор `HistoryStore(directory, driver, …)`;
> параметра-подменыша у `openOrNull` нет. Смысл шва в том, чтобы тест получил **ту же**
> конфигурацию, а не возможность подсунуть свою, поэтому все тесты открывают стор через `openOrNull`
> над временным `home` из `mkdtemp`.
>
> ⚠️ **`openHistoryDriver(directory)` заводилась публичной и стала `private` во внешнем ревью после
> задачи 14** (`HistoryStore.kt:336`) — ровно по этому же основанию: публичная фабрика драйвера и
> есть возможность подсунуть свою конфигурацию, а её тестам никто не давал. Ни один тест её не звал,
> так что снятие видимости ничего не стоило.
...
- [x] написать тест отката: сорвать запись предсказуемо — `driver.execute(null, "DROP TABLE application_sample", 0)` перед `record` — и убедиться, что в базе не осталось ни строки этого сэмпла; ⚠️ роняем `alert`, а не `application_sample`, см. заметку ниже
...
>
> ⚠️ **Таблица отката заменена: `alert` вместо `application_sample`** (`HistoryStoreRecordTest.kt:131`).
> `application_sample` читает сам ретеншн (`deleteOrphanApplications`), а он идёт до транзакции и
> отрабатывает на первом же сэмпле каждого запуска — сорвался бы не откат, а пасс ретеншна, и
> `countRows("application_sample")` после этого тоже некому было бы ответить. `alert` ретеншн не
> называет, поэтому падение приходится ровно на середину транзакции, как и задумано. Первый сэмпл
> пишется до дропа, чтобы счётчикам было от чего отличаться.
...
> Счётчик сэмплов инкрементируется до попытки, так что упавший проход не сдвигает расписание
> следующих.
>
> ⚠️ **Цена упавшего прохода пересмотрена во внешнем ревью после задачи 14.** Здесь было записано,
> что упавший проход стоит одного сэмпла; теперь он не стоит и его. `pruneIfDue`
> (`HistoryStore.kt:265-273`) ловит исключение сам и отправляет его в `logError`: ретеншн
> освобождает место, а запись — это то мониторинговое наблюдение, ради которого агент существует, и
> упавший `incremental_vacuum` не должен пробивать в ряду часовую дыру с сообщением
> «history write failed», указывающим не туда. Счётчик по-прежнему двигается: ретеншн, падающий
> раз за разом, стоит часа невозвращённого места на проход, а не прохода — и падения — на каждом
> сэмпле после первого.
>
