...
- [ ] добавить в `alertsFor` правило по `usage.processes` с условием `reparentedFrom != null && parentPid == 1`
- [ ] сформировать ключ `orphan:process:<pid>:<startedAt>` и сообщение с именем и pid умершего родителя
- [ ] выставить `Severity.WARNING` безусловно, KDoc **на английском** объясняет, почему `CRITICAL` здесь неприменим
- [ ] реализовать cap инлайн: отсортировать по pid, `take(maxAlertsPerCategory)`, остаток в `suppressed`; **не** пытаться переиспользовать `selectAlerting` — он типизирован под `ApplicationUsage` и требует числовой метрики
- [ ] KDoc отмечает, что подавленный orphan теряется навсегда: `firingKeys` держит подавленный ключ только если тот уже был активен, а edge второй раз не наступит
- [ ] написать тест: один осиротевший процесс даёт один алерт с ожидаемым ключом и текстом
- [ ] написать тест: переход к родителю, отличному от 1, алерта **не** даёт
- [ ] написать тест: родитель без имени даёт сообщение с голым pid
- [ ] написать тест: 5 сирот при `maxAlertsPerCategory = 3` дают 3 алерта по наименьшим pid и 2 подавленных ключа
- [ ] написать тест: процесс без перехода алерта не даёт
- [ ] написать тест в `ReportFormatterTest`: текстовый отчёт проносит orphan-алерт через общий блок `alerts` (`ReportFormatter.kt:105-116`) — своего кода рендера не требуется
- [ ] `./kotlin build && ./kotlin test` — должны пройти до задачи 4
...
- [ ] добавить приватную функцию, читающую `PRAGMA table_info(process)` и возвращающую набор имён колонок
- [ ] добавить выполнение `ALTER TABLE process ADD COLUMN reparented_at TEXT`, когда колонки нет
- [ ] вызвать миграцию в новом `init` блоке `HistoryStore` (сегодня его нет), после создания `database`
- [ ] **не** оборачивать в `runCatching`: sqliter печатает полный стектрейс до броска, проглоченное исключение всё равно даст стену красного в launchd-логе на каждом старте агента
- [ ] написать KDoc **на английском**: это первая миграция в проекте и почему `.sqm` для неё непригоден
- [ ] написать хелпер, создающий базу **старой** формы: таблицу `process` построить сырым SQL по прежнему определению — это документирует дошаговую форму схемы там, где её больше нигде не видно
...
- [ ] переиспользовать существующий `withScratchHome` (`history-sqlite/test/TestFixtures.kt:359`), а не писать свой
- [ ] написать тест: открытие store над базой старой формы добавляет колонку, `PRAGMA table_info` её показывает
- [ ] написать тест: данные, записанные до миграции, уцелели и читаются
- [ ] написать тест идемпотентности: повторное открытие не пытается добавить колонку снова и не бросает
- [ ] написать тест: открытие пустой директории создаёт базу сразу с колонкой (путь `Schema.create`, без миграции)
- [ ] `./kotlin build && ./kotlin test` — должны пройти до задачи 6
