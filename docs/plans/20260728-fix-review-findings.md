...
➕ Срезы живут в классе `ApplicationRankings` (файл `report/ApplicationRankings.kt`), а не внутри
форматтеров: он задаёт каждый ранжированный срез ровно один раз, поэтому определения не
дублируются между `ReportFormatter.text` и `ReportJson`. Каждый срез — `by lazy`: рендер только
текстового отчёта не платит за срезы, нужные лишь JSON-полезной нагрузке.

➕ Правка ревью после задачи 17 убрала параметр `rankings` из всех шести публичных сигнатур
(`text`, `json`, `notification`, `encode` и двух вызывающих в `HarmonService`): проброс был
чистой сантехникой ради экономии четырёх сортировок ~100 приложений на уведомляемый сэмпл.
`ReportFormatter.text` и `ReportJson.toDto` теперь каждый строит свой `ApplicationRankings`,
то есть уведомление ранжирует дважды — ровно как до ветки. Дедупликация определений срезов,
ради которой класс и вводился, сохранена.
