...
- **предпосылка, проверяемая в задаче 1**: тестовый сорс-сет Amper должен видеть `internal`
  из `src/`. Если нет — хелперы, вынесенные ради тестируемости, делаются публичными
...
- [ ] убедиться, что тест из `test/` видит `internal`-объявление из `src/` (пробный тест на
      `HtmlReportStore`); если нет — зафиксировать ⚠️ и делать хелперы публичными
- [ ] написать тест: ключ, отсутствовавший в прошлом сэмпле, попадает в `newlyActive`
- [ ] написать тест: ключ, доставленный и подтверждённый `commit`, в `newlyActive` не попадает
- [ ] написать тест: ключ, погасший на сэмпле, при следующем загорании снова считается новым
- [ ] написать тест: свежий ключ с пустым `deliveredKeys` остаётся новым на следующем сэмпле
- [ ] написать тест: `activeKeys` содержит ключ с провалившейся доставкой (гистерезис не выключается)
- [ ] написать тест: оба множества не растут при потоке одноразовых ключей `process:<pid>:<startedAt>`
- [ ] реализовать `AlertState` с `newlyActive`, `commit`, `activeKeys`
- [ ] запустить `./kotlin test` — должно пройти до перехода к задаче 2
...
- [ ] написать тест: значение между 90% и 100% порога даёт алерт, если ключ был в `activeKeys`
- [ ] написать тест: то же значение без `activeKeys` алерта не даёт
- [ ] написать тест: значение ниже 90% порога гасит алерт даже при наличии ключа в `activeKeys`
- [ ] написать тест: `severity` считается от исходного порога, а не от сниженного
- [ ] написать тест: `battery-low` при 19% и пороге 20 гаснет, а не удерживается (гистерезис не применён)
- [ ] написать тест: 4 приложения над порогом при `maxAlertsPerCategory=3` — активный ключ,
      вытесненный из top-3, остаётся в списке алертов
- [ ] добавить параметр `activeKeys: Set<String> = emptySet()` и константу `CLEAR_RATIO = 0.9`
- [ ] применить гистерезис ко всем категориям, кроме `battery-low`
- [ ] дополнять срез top-N активными ключами категории (потолок `2 × maxAlertsPerCategory`)
- [ ] обновить таблицу порогов в `docs/collection.md:330-347` — описать гистерезис
- [ ] запустить `./kotlin test` — должно пройти до перехода к задаче 3
...
- [ ] написать тест на `internal fun classifyAccept(result, consecutiveFailures)`:
      `>= 0` → принять и сбросить счётчик, `-2` → отклонить без инкремента счётчика,
      `-1` → залогировать, инкрементировать, продолжить
- [ ] написать тест: N подряд отказов авторизации (`-2`) не приводят к фатальному исходу
- [ ] написать тест: `CONSECUTIVE_ACCEPT_FAILURE_LIMIT` подряд ошибок `-1` даёт фатальный исход
- [ ] вынести классификацию в `internal` функцию и покрыть её тестами
- [ ] заменить `throw` в ветке `else` на логирование и продолжение цикла
- [ ] добавить паузу `hm_sleep_millis` между подряд идущими ошибками
- [ ] запустить `./kotlin test` — должно пройти до перехода к задаче 11
...
- [ ] написать тест: payload с `protocolVersion = 2` и неизвестным полем даёт сообщение про
      неподдерживаемую версию, а не про невалидный JSON
- [ ] написать тест: payload версии 1 с неизвестным полем по-прежнему отвергается
- [ ] написать тест: корректный payload версии 1 декодируется без изменений
- [ ] написать тест: невалидный JSON даёт `CollectorProtocolException` про невалидный JSON
- [ ] парсить payload один раз в `JsonElement`, читать `protocolVersion`, декодировать через
      `decodeFromJsonElement` (двойного прохода по кадру до 32 МиБ быть не должно)
- [ ] запустить `./kotlin test` — должно пройти до перехода к задаче 12
