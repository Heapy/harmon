...
- Modify: `src/dev/yoda/harmon/monitor/DarwinSystemCollector.kt`
- Modify: `docs/collection.md`
- Create: `test/DarwinCollectorLimitsTest.kt`

- [x] добавить параметр `attribution_region_budget` в `hm_list_processes` и расходовать его в цикле атрибуции
- [x] процесс, обход которого оборвался по лимиту или бюджету, не помечать
      `compressed_attribution_available` (заниженная сумма не должна выглядеть измеренной)
- [x] процесс, до которого бюджет не дошёл, не помечать `compressed_attribution_attempted`
      (иначе `compressedAttributionFailureCount` считает несделанные попытки)
- [x] снизить рабочий per-process лимит до 8192, оставив `HM_MAX_VM_REGIONS` предохранителем
- [x] добавить конструкторный параметр `attributionRegionBudget` (дефолт 100000) в `DarwinSystemCollector`
- [x] обновить описание счётчиков атрибуции в `docs/collection.md`
- [x] проверить вручную: `DarwinSystemCollector(attributionRegionBudget = 0).capture()` даёт
      `compressedAttributionProcessCount == 0` и `compressedAttributionFailureCount == 0`
      при непустом списке процессов
- [x] проверить вручную на машине с запущенным браузером: `time harmon diagnose` укладывается
      в единицы секунд
- [x] запустить `./kotlin test` — должно пройти до перехода к задаче 17

➕ `hm_read_compressed_or_paged_out` вернула трёхзначный статус (`0` — обход дошёл до конца,
`1` — оборван лимитом, `-1` — процесс не читается) и out-параметр `consumed_regions`. Без
разделения «оборван» и «дошёл» первый требуемый инвариант не выражается вовсе: старый код
возвращал `0` в обоих случаях. Без `consumed_regions` бюджет пришлось бы списывать по
`virtual_memory_region_count`, который на неудачном чтении не выставляется, и провалившиеся
процессы обходились бы бесплатно.

➕ Обход, закончившийся ровно на последнем разрешённом регионе, считается оборванным, даже если
адресное пространство закончилось там же. Смещение одностороннее и намеренное: пропущенное
измерение вернётся на следующем сэмпле, выдуманное — нет.

➕ `test/DarwinCollectorLimitsTest.kt` сверх списка файлов задачи: логика бюджета живёт в C и из
тестов недостижима (⚠️ задачи 9), но новый конструкторный параметр — код на Kotlin, и его
контракт (`-1` отвергается с внятным сообщением, `0` принимается как способ выключить атрибуцию)
покрывается. Конструктор `DarwinSystemCollector` линкуется из тестов: `require` не заходит в
`nativebridge`, как и `processCapacityFor` в задаче 15.

⚠️ Ручная проверка — честный A/B на живой машине с Firefox, IntelliJ (2.8 ГБ RSS) и работающей
VM. Pre-change бинарь собран из `git stash`, оба коллектора подняты одновременно на разных
сокетах, `diagnose --sample-seconds 2` прогнан по три раза вперемежку. Время (2 с из них — окно
сэмпла): **3.40–3.45 с → 2.76–2.78 с**, то есть сбор одного снимка **≈0.70 с → ≈0.39 с**. Число
процессов не изменилось (460/681 readable у обоих). Оба граничных случая проверены отдельными
сборками с подменённым дефолтом бюджета: при `attributionRegionBudget = 0` — `measured 0 …
0 attempts failed` при 459 читаемых процессах (ни один кандидат не помечен attempted); при
бюджете 64 — `measured 0 … 1 attempts failed` (единственный дошедший процесс оборван на 64-м
регионе, посчитан провалом, остальные 255 кандидатов не тронуты).

⚠️ Побочный эффект, которого в плане нет: покрытие атрибуции на нагруженной машине падает с
**256 процессов до ~55** (`measured 55 of 460; 6 attempts failed` против `measured 256 of 460;
0 attempts failed`). Бюджет 100000 съедается несколькими десятками крупнейших процессов, а
шесть провалов — это обходы, обрезанные новым per-process лимитом 8192. Это ровно тот размен,
ради которого задача делалась (худший случай 8.4 млн `proc_pidinfo` на сбор), но цифра заметная,
поэтому она зафиксирована в `docs/collection.md`, а не осталась сюрпризом в логах.

### Task 17: Считать ранжирование приложений и процессов один раз

**Files:**
- Create: `src/dev/yoda/harmon/report/ApplicationRankings.kt`
- Modify: `src/dev/yoda/harmon/report/ReportFormatter.kt`
- Modify: `src/dev/yoda/harmon/report/ReportJson.kt`
- Modify: `src/dev/yoda/harmon/runtime/HarmonService.kt`
- Modify: `test/ReportFormatterTest.kt`
- Modify: `test/ReportJsonTest.kt`
- Modify: `test/TestFixtures.kt`

- [x] написать тест: при равных значениях метрики порядок элементов совпадает с
      `sortedByDescending { }.take(n)` (защита от разъезжания порядка)
- [x] написать тест: вывод `text` и `json` на фикстуре не меняется после правки
      (эталон — строковая константа, снятая до изменения)
- [x] вычислять ранжированные срезы один раз на отчёт и переиспользовать в обоих форматтерах
- [x] сохранить `sortedByDescending` как механизм сортировки — самописная частичная выборка
      не пишется: выигрыш микросекундный, риск изменить порядок при равных значениях реальный
- [x] запустить `./kotlin test` — должно пройти до перехода к задаче 18

➕ Срезы живут в новом публичном классе `ApplicationRankings` (файл `report/ApplicationRankings.kt`),
а не внутри форматтеров: он принимается обоими как параметр со значением по умолчанию
(`text(report, rankings)`, `json(report, newAlertKeys, rankings)`, `notification(report,
highlighted, rankings, reportText)`), поэтому `once`/`diagnose` продолжают работать без изменений,
а вызывающий, которому нужны оба рендера, ранжирует один раз. Каждый срез — `by lazy`: рендер
только текстового отчёта не платит за срезы, нужные лишь JSON-полезной нагрузке.

➕ `HarmonService` в списке файлов задачи не значился, но правка без него бессмысленна для агента:
`handleSample` рендерит текст сам и передаёт его в `notification` как `reportText`, так что без
проброса `rankings` через `deliverIfNeeded`/`deliverSample` четыре общих сортировки остались бы
удвоенными ровно на горячем пути. Один `ApplicationRankings` на сэмпл строится в `handleSample`.

➕ Ранжирование процессов (7 срезов в `ReportJson`) намеренно оставлено на месте: оно считается
один раз и в обоих форматтерах не дублируется, а перенос дал бы только churn. Поэтому класс
назван `ApplicationRankings`, а не `ReportRankings`.

➕ Эталон для `json` — не полный документ (≈10 КБ одной строкой, нечитаемый диф), а сводка
`секция.срез=имена` по всем 14 ранжированным срезам (`SLICES_GOLDEN` в `ReportJsonTest`): она
фиксирует ровно то, что правка могла сломать — состав и порядок каждого среза. Эталон для `text` —
полный вывод как есть (`TEXT_GOLDEN`), снятый до правки. Оба сняты и прогнаны на неизменённом коде,
и остались зелёными после.

➕ Тест `ranksASliceOnceAndHandsTheSameListToEveryReader` (`assertSame` на повторном чтении среза)
сверх чеклиста: без него «один раз на отчёт» тестом не проверяется — эталоны прошли бы и при
пересортировке на каждое обращение.

⚠️ Двойного рендера `text(report)` в `notification` нет (снят в задаче 4): HTML собирается из
параметра `reportText`, а `text(report, rankings)` остался только его значением по умолчанию.
...
- [x] обновить README.md: секция уведомлений — событийная семантика, новые ключи конфига
...
порог, отсутствие напоминаний, повторный пуш после погасания, гистерезис 90% с исключением для
`battery-low`, best-effort статус Notification Center и повтор при провале решающих каналов,
расхождение «пуш = новые алерты, HTML/JSON = все активные + `newAlertKeys`», поведение
`notifyEverySample`. Ключи конфига (`terminalApplications`, deprecated `alertCooldownSeconds`) уже
были на месте после задач 6 и 13 — проверено, дублей не добавлено.

Упоминаний кулдауна вне намеренного абзаца про принимаемый-и-игнорируемый ключ нет:
`grep -rni cooldown` по `*.md`/`*.kt`/`*.def`/`config/` даёт только `README.md:189`,
`config/Config.kt:138` (`deprecatedKeys`) и три строки в `ConfigLoaderTest`.

`docs/architecture.md`: последний шаг sequence-диаграммы «Notify only when configured and due»
(таймерная формулировка) заменён на «Push only the alerts that just crossed a threshold»; добавлен
абзац про edge-triggered доставку с двумя множествами ключей и про монотонный сон; в Failure
behavior добавлены исход отказа `accept` (16 подряд, отказ авторизации бюджет не тратит),
переживание исключения в `handleSample` и правило «ключ считается доставленным только после
подтверждения решающим каналом».

➕ Два расхождения кода и docs, найденных сверкой и исправленных сверх чеклиста:
`docs/collection.md:58` утверждал фиксированную ёмкость 16 384 / 4 096 записей — после задачи 15
это потолок, а реальная аллокация считается от живого числа PID; `README.md:72` описывал только
лимит в 256 процессов, без бюджета в 100 000 регионов из задачи 16 и без падения покрытия на
нагруженной машине. Оба места переписаны по коду.

➕ `docs/collection.md` («JSON and privacy»): в состав webhook-пейлоада добавлено `newAlertKeys` —
поле аддитивное и видимо потребителям, а список содержимого его не упоминал.

⚠️ Конверсия CPU-времени из mach absolute time (дефект из задачи 18) в `docs/collection.md:265-268`
уже описана верно и соответствует `hm_mach_time_to_ns` в `cinterop/harmon_native.def:515`;
описания старого поведения в документации не осталось.

➕ `CLAUDE.md` создан: два ограничения, проверенных эмпирически в задачах 1 и 9 (тестовый сорс-сет
не видит `internal` из `src/`; тестовый бинарь не линкует cinterop-клиб, поэтому `nativebridge`
из тестов недостижим, а partial linkage резолвится по месту вызова, а не по файлу), плюс
`allWarningsAsErrors` как источник падений сборки на unused import, конвенция DI через
конструкторные параметры с дефолтами и правило «документация описывает поведение, а не намерение».
Из сессии это единственное знание, которое нельзя вывести чтением исходников.
