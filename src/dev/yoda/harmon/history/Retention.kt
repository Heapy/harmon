...
/**
 * Whether the sample numbered [sampleIndex] — counted from zero within this run of the agent — is the
 * one that carries the retention pass.
 *
 * The schedule is expressed in samples because that is the only clock the write path has, and it is
 * derived from [PRUNE_PERIOD_SECONDS] rather than fixed at a sample count: at the default interval of
 * 300 seconds every twelfth sample prunes, at 30 seconds every hundred and twentieth. The division
 * rounds up, so the gap never falls below the period and never exceeds it by more than one interval —
 * and an interval longer than an hour lands on one sample per pass instead of on zero, which is what
 * a rounding-down division would produce.
 *
